...
module.exports = {
  ...
  fn: function parseNativeQueryResult(inputs, exits) {
    ...
    switch (inputs.queryType) {
      case 'select':
        normalizedResult = inputs.nativeQueryResult.recordsets[0];
        break;

      case 'insert':
        var inserted;
        var insertedId;
        if (inputs.nativeQueryResult.recordsets.length === 2) { // inserted rows and identity field both returned
          inserted = inputs.nativeQueryResult.recordsets[0];
          insertedId = Object.values(inputs.nativeQueryResult.recordsets[1][0])[0];
        }
        if (inputs.nativeQueryResult.recordsets.length === 1) { // inserted rows OR identity field  returned. We'll have to guess which one unless specified in meta.
          if (inputs.meta.hasIdentity || Object.keys(inputs.nativeQueryResult.recordsets[0][0])[0] === '') {
            insertedId = Object.values(inputs.nativeQueryResult.recordsets[0][0])[0];
          }
          else {
            inserted = inputs.nativeQueryResult.recordsets[0];
          }
      ...
      case 'update':
        normalizedResult = {
          recordsUpdated: inputs.nativeQueryResult.recordsets[0],
          numRecordsUpdated: inputs.nativeQueryResult.rowsAffected[0]
        };
        break;

      case 'delete':
        normalizedResult = {
          recordsDeleted: inputs.nativeQueryResult.recordsets[0],
          numRecordsDeleted: inputs.nativeQueryResult.rowsAffected[0]
        };
        break;
