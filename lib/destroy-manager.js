module.exports = {
  ...
  fn: function destroyManager(inputs, exits) {
    ...
    //
    // For more info, see:
    //  • https://github.com/felixge/node-mssql/blob/v2.10.2/Readme.md#closing-all-the-connections-in-a-pool
    inputs.manager.pool.close(function close(err) {
      if (err) {
        return exits.failed({
          error: new Error('Failed to destroy the MSSQL connection pool and/or gracefully end all connections in the pool.  Details:\n=== === ===\n' + err.stack),
          meta: inputs.meta
        });
      }
