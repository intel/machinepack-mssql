  ...
  fn: function createManager(inputs, exits) {
    ...
    if (!_.isUndefined(inputs.meta)) {
      ...
      [
        ...
        'debug', 'trace', 'multipleStatements', 'flags', 'options', 'authentication',
    ...
    try {
      ...
      if (_.isString(parsedConnectionStr.pathname)) {
        ...
        if (parsedConnectionStr.pathname.match(/\/.+\/.+/)) {
          ...
          _databaseName = parsedConnectionStr.pathname.split(/\//)[2];
        }
        else {
          _databaseName = parsedConnectionStr.pathname;
