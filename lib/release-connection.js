  ...
  fn: function releaseConnection(inputs, exits) {
    ...
    try {
      ...
      try {
        ...
        return exits.error(new Error('Could not release MSSQL connection gracefully, and attempting to forcibly destroy the connection threw an error.  Details:\n=== === ===\n' + _destroyErr.stack + '\n\nAnd error details from the original graceful attempt:\n=== === ===\n' + _releaseErr.stack));
      }

      // Otherwise if we're here, while we could not release the MSSQL connection
