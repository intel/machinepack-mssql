module.exports = {
  ...
  moreInfoUrl: 'http://dev.mssql.com/doc/refman/5.7/en/commit.html',
  ...
  fn: async function rollbackTransaction(inputs, exits) {
    if (!inputs.connection.currentTransaction) {
      return exits.error(new Error('Transaction is gone. can\'t roll back'));
    }
    if (!inputs.connection.currentTransaction.rolledBack) {
      await inputs.connection.currentTransaction.rollback();
    }
